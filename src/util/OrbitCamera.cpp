...
void OrbitCamera::Update(float dt, const InputManager& input)
{
    ...
    // Arrow keys: orbit
    float orbitRate = m_keyOrbitSpeed * dt;
    if (input.IsKeyDown(VK_LEFT))  m_yaw   -= orbitRate;
    if (input.IsKeyDown(VK_RIGHT)) m_yaw   += orbitRate;
    if (input.IsKeyDown(VK_UP))    m_pitch += orbitRate;
    if (input.IsKeyDown(VK_DOWN))  m_pitch -= orbitRate;
    ...
    // +/- keys: zoom
    float zoomDelta = m_keyZoomSpeed * dt;
    if (input.IsKeyDown(VK_OEM_PLUS))  m_distance -= zoomDelta;
    if (input.IsKeyDown(VK_OEM_MINUS)) m_distance += zoomDelta;

                                    ...
                                    D3D12_DESCRIPTOR_HEAP_TYPE type)
{
    ...
    ASSERT_SUCCEEDED(device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&m_heap)));
...
D3D12_CPU_DESCRIPTOR_HANDLE PersistentDescriptorHeap::Allocate(UINT& outIndex)
{
    ...
    {
        assert(m_nextFreeSlot < m_capacity && "PersistentDescriptorHeap: out of descriptors");
...
D3D12_GPU_DESCRIPTOR_HANDLE TransientDescriptorHeap::CopyDescriptors(
    ID3D12Device* device, const D3D12_CPU_DESCRIPTOR_HANDLE* srcHandles, UINT count)
{

    assert(m_currentOffset + count <= m_capacity && "TransientDescriptorHeap: out of descriptors");
